...
select
  a.auth_ts,
  a.user_reference,
  array_agg(lower(merchant_name) order by b.auth_ts limit {{seq_len}}) merchant_name,
  array_agg(cast(day_of_week as int64) order by b.auth_ts limit {{seq_len}}) day_of_week,
  array_agg(cast(eighth_of_day as int64) order by b.auth_ts limit {{seq_len}}) eighth_of_day,
  array_agg(cast(amount as float64) order by b.auth_ts limit {{seq_len}}) amount,
  array_agg(sys_category order by b.auth_ts limit {{seq_len}}) sys_category,
  array_agg(b.auth_ts order by b.auth_ts limit {{seq_len}}) auth_ts_seq
from transactions a
join
  (select
    user_reference,
  ...
  and amount*-1 > 0
  and amount*-1 < 3000
  and format_date('%Y-%m', date(auth_ts)) = '2020-02') b
